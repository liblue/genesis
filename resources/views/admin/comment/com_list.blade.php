...
<section class="Hui-article-box">
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="text-c">
				<form action="{{url('admin/comment/com_list')}}" method="get">
                                    <input  type="hidden" name="id"  value="{{$pre_id}}">
				...
				<input type="text" name="content" id="" placeholder=" 评论内容" style="width:250px" class="input-text">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				...
				<a href="javascript:;" onclick="delAll()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
                                 ...
                                 <a href="{{url('admin/article/'.$data->id)}}"><span  style="vertical-align:middle;">{{$data->title}}</span></a>
				
				</span>
                         
				<span class="r">共有数据：<strong>{{count($data->comment)}}</strong> 条</span>
			</div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<tbody>
                                            ...
                                            @foreach($data->comment as $k=>$value)
						<tr class="text-c">
							<td><input type="checkbox"  name="del"  value="{{$value->id}}"></td>
							<td>{{$value->id}}</td>
							<td><a href="{{url('admin/comment/com_list').'?id='.$value->id.'&type=comment'}}" >{{$value->content}}</a></td>
							<td width="80">{{$value->user->name}}</td>
	...
	<td class="f-14 td-manage">
                                    ...
                                    @if(in_array('评论删除',$role))                    
                                    <a style="text-decoration:none" class="ml-5" onClick="article_del(this,{{$value->id}})" href="javascript:;" title="删除">
...
function article_del(obj,id){
       ...
       layer.confirm('确认要删除吗？',function(index){
		//此处请求后台程序，下方是成功后的前台处理……
	                $.post("{{url('admin/comment/delete')}}",{'_token':'{{csrf_token()}}','data':id},function(data){
                       if(data==="没有权限"){
