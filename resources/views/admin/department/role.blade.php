...
<dl id="menu-department">
			...
			<dd   style="display:block">
				<ul>
					...
					<li  class="current"><a href="{{url('admin/role')}}" title="部门列表">角色列表</a></li>
					@endif
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					<thead  class="header">
						<tr class="text-c">
							...
							<th width="50">创建时间</th>
							  @if(in_array('角色编辑',$role) || in_array('角色删除',$role) )         
							<th width="120">操作</th>
                                                        @endif
						</tr>
					</thead>
					<tbody>
						...
						<tr class="text-c">
							...
							<td width="80">{{$value->name}}</td>
                                  ...
                                  @if(in_array('角色修改',$role) || in_array('角色删除',$role) )                     
...
function depart_edit(obj,id){
	...
	layer.prompt({title: '请输入修改过后的角色名称，并确认'}, function(val, index){         
        layer.close(index);
        if(val.length>10){
       layer.msg('不能超过十个字符'); 
     return false;
   }
     $.post("{{url('admin/role/set')}}",{'_token':'{{csrf_token()}}','data':val,'id':id},function(data){
           if(data=="修改成功"){
...
function depart_del(obj,id){
       layer.confirm('确认要删除吗？',function(index){
		//此处请求后台程序，下方是成功后的前台处理……
	                $.post("{{url('admin/role/delete')}}",{'_token':'{{csrf_token()}}','data':id},function(data){
        ...
        layer.confirm('确认要删除吗？',function(index){         
             $('input:checkbox[name=del]:checked').each(function(index){
                 ...
                 var  id=$(this).val();
                     ...
                     $.post("{{url('admin/role/delete')}}",{'_token':'{{csrf_token()}}','data':id},function(data){
                    if(data=="此角色存在用户"){
                    layer.msg('此角色存在用户!',{icon:2,time:1000});}
                    else if(data=="没有权限"){
                    location.href="{{url('admin/manage')}}";
                     }
                      else{
